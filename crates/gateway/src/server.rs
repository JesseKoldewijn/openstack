...
impl Gateway {
    ...
    /// Constructs an Axum Router configured for this gateway.
    ///
    /// The returned router is wired to the gateway's shared AppState (including config,
    /// plugin manager, and CORS handler), uses the gateway's request fallback handler,
    /// and applies response compression.
    ///
    /// # Examples
    ///
    /// ```no_run
    /// let gateway = /* obtain or construct a Gateway instance */;
    /// let router = gateway.build_app();
    /// // router can now be served or used in tests
    /// ```
    ...
    /// Builds the axum Router configured for this Gateway for use in tests.
    ///
    /// # Examples
    ///
    /// ```
    /// let gateway = Gateway::new(config, plugin_manager);
    /// let app = gateway.build_app_for_tests();
    /// // `app` can be used with hyper::Server::bind or axum::test::TestClient
    /// ```
    #[doc(hidden)]
...
/// Handles all incoming HTTP requests to the gateway, routing them to the Studio SPA, internal APIs, or the configured service plugins.
///
/// The handler:
/// - serves Studio SPA shell and static Studio assets when their routes are requested,
/// - responds to CORS preflight requests,
/// - routes internal API requests under /_localstack/ to the internal API handler,
/// - builds a RequestContext from the HTTP request (including signature, service detection, protocol parsing, and parameters),
/// - dispatches the request to the service plugin manager and serializes the provider response (buffered or streaming),
/// - attaches provider and CORS headers to the final HTTP response.
///
/// # Examples
///
/// ```
/// use axum::Router;
///
/// // Mount the gateway handler as a fallback for all routes:
/// let app = Router::new().fallback(crate::handle_request);
/// ```
...
/// Checks whether a request path targets the Studio single-page application.
///
/// Returns `true` if the path is the Studio SPA root ("/_localstack/studio" or "/_localstack/studio/")
/// or any subpath under "/_localstack/studio/", `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(is_studio_spa_route("/_localstack/studio"));
/// assert!(is_studio_spa_route("/_localstack/studio/"));
/// assert!(is_studio_spa_route("/_localstack/studio/app"));
/// assert!(!is_studio_spa_route("/_localstack/other"));
/// ```
...
/// Checks whether a request path targets a Studio static asset (either the SPA's app.js or app.css).
///
/// # Returns
///
/// `true` if the path is exactly "/_localstack/studio/assets/app.js" or "/_localstack/studio/assets/app.css", `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(is_studio_asset_route("/_localstack/studio/assets/app.js"));
/// assert!(is_studio_asset_route("/_localstack/studio/assets/app.css"));
/// assert!(!is_studio_asset_route("/_localstack/studio/"));
/// ```
fn is_studio_asset_route(path: &str) -> bool {
    path == "/_localstack/studio/assets/app.js" || path == "/_localstack/studio/assets/app.css"
}

/// Builds an HTTP 200 response that serves the embedded Studio single-page application shell.
///
/// The response has:
/// - Status 200 OK
/// - `Content-Type: text/html; charset=utf-8`
/// - `Cache-Control: no-cache`
/// - `ETag: "studio-shell-v1"`
/// - Body set to the `STUDIO_SPA` payload.
///
/// # Examples
///
/// ```
/// let resp = studio_spa_response();
/// assert_eq!(resp.status(), http::StatusCode::OK);
/// let headers = resp.headers();
/// assert_eq!(headers.get("content-type").unwrap(), "text/html; charset=utf-8");
/// assert_eq!(headers.get("cache-control").unwrap(), "no-cache");
/// assert_eq!(headers.get("etag").unwrap(), "\"studio-shell-v1\"");
/// ```
...
/// Serve a Studio static asset for the given request path.
///
/// Returns a Response containing the asset body (app.js or app.css) with
/// appropriate Content-Type and long-term caching headers, or a plain-text
/// "Not found" body for unknown asset paths.
///
/// # Parameters
///
/// - `path`: the request path to resolve into a Studio asset (e.g. "/_localstack/studio/assets/app.js").
///
/// # Returns
///
/// A `Response` with status 200 and the asset payload or `"Not found"`; includes
/// `content-type`, `cache-control`, and `etag` headers.
///
/// # Examples
///
/// ```
/// let resp = studio_asset_response("/_localstack/studio/assets/app.js");
/// assert_eq!(resp.status(), http::StatusCode::OK);
/// assert!(resp.headers().get("content-type").unwrap().to_str().unwrap().starts_with("application/javascript"));
/// ```
...
/// Builds a RequestContext by extracting authentication, service, region, protocol, operation, and parameters from an incoming HTTP request.
///
/// The function:
// - Parses SigV4 Authorization (if present) to derive access key, region, and service; falls back to defaults when missing or unparsable.
// - Derives an account ID from the access key.
// - Detects the target service from authorization, host, path, headers, query, and body heuristics.
/// - Validates and normalizes the region, falling back to `us-east-1` unless `config.allow_nonstandard_regions` is set.
/// - Determines the AWS protocol for the service and parses the operation name and parameters according to that protocol.
/// - Returns a populated `RequestContext` on success, or an HTTP `Response` with status 400 when request parsing fails.
///
/// # Parameters
///
/// - `config`: used to decide whether nonstandard regions are allowed (`allow_nonstandard_regions`).
///
/// # Returns
///
/// `RequestContext` on success; an HTTP `Response` with status 400 if operation/parameter parsing fails.
///
/// # Examples
///
/// ```no_run
/// use std::collections::HashMap;
/// use bytes::Bytes;
/// // Assume `Config`, `SpooledBody`, `RequestContext` and `build_request_context` are in scope.
/// let method = &"POST".parse().unwrap();
/// let path = "/";
...
/// let spooled_body = SpooledBody::default();
///
/// match build_request_context(method, path, &query_params, &headers, &body, request_id, &config, spooled_body) {
///     Ok(ctx) => { /* dispatch using ctx */ }
///     Err(resp) => { /* return error response */ }
/// }
/// ```
...
/// Proxy requests under /_localstack/* to the internal API router and return the routed response.
///
/// If the path is a Studio guided-execution endpoint, validates that the method is POST and that the
/// request body does not exceed STUDIO_GUIDED_MAX_PAYLOAD_BYTES; otherwise returns an appropriate
/// error response. Rebuilds an Axum request from the provided path, method, headers, query params,
/// and body, dispatches it to the internal router, and returns the router's response or a 500 on
/// dispatch failure.
///
/// # Examples
///
...
///
/// # async {
/// // This is a usage example; providing a real AppState is required to actually run.
/// let _resp = crate::server::handle_internal_api(
///     "/_localstack/health".into(),
///     &Method::GET,
///     &HashMap::new(),
///     &HashMap::new(),
///     &Bytes::new(),
...
/// Checks whether the request path targets the Studio guided execution endpoints.
///
/// Returns `true` if the path is "/_localstack/studio-api/flows/execute" or
/// "/_localstack/studio-api/flows/replay", `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(is_studio_guided_execution_route("/_localstack/studio-api/flows/execute"));
/// assert!(is_studio_guided_execution_route("/_localstack/studio-api/flows/replay"));
/// assert!(!is_studio_guided_execution_route("/_localstack/other"));
/// ```
