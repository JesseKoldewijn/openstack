...
/// Starts a TestHarness, verifies Studio HTTP endpoints and headers, exercises theme persistence, and shuts down the harness.
///
/// Performs GET requests to /_localstack/studio, /_localstack/studio-api/services, /_localstack/studio-api/flows/catalog, and /_localstack/studio-api/flows/coverage and asserts each returns HTTP 200. Verifies the /_localstack/studio response includes `Cache-Control: no-cache`. Uses `openstack_studio_ui::ThemeStore` to toggle theme from Light to Dark and asserts `storage_value()` equals `"dark"`, then shuts down the harness.
///
/// # Examples
///
/// ```
...
/// let res = harness.client.get(harness.url("/_localstack/studio")).send().await.unwrap();
...
/// Runs a guided S3 workflow through the Studio API, verifies the object was created with the expected contents, and cleans up created resources.
///
/// This test starts the harness with S3 enabled, executes each step of a prebuilt S3-guided workflow via the Studio API client, asserts each step succeeded (HTTP 200), retrieves the created object to verify its contents, deletes the object and bucket, and then shuts down the harness.
///
/// # Examples
///
/// ```
...
/// let harness = TestHarness::start_services("s3").await;
/// let client = openstack_studio_ui::StudioApiClient::new(harness.base_url.clone());
/// let workflow = openstack_studio_ui::GuidedWorkflow::s3_basic("studio-guided-bucket", "k.txt", "hello");
///
...
/// Executes a guided SQS workflow against a test harness and verifies the target queue is created.
///
/// The test starts a harness with SQS enabled, runs each step of the guided workflow via the
/// Studio API client (asserting each step returns HTTP 200), then performs an AWS ListQueues
/// request to confirm the queue named "studio-flow-queue" exists.
///
/// # Examples
///
/// ```
...
///     // Runs the end-to-end guided SQS flow and assertions.
///     studio_guided_sqs_flow().await;
/// }
/// ```
#[tokio::test]
