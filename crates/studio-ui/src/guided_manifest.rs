use std::collections::{HashMap, HashSet};
use std::path::Path;

use serde::{Deserialize, Serialize};
use serde_json::Value;
use thiserror::Error;

pub const SUPPORTED_SCHEMA_VERSION: &str = "1.2";
pub const GUIDED_MANIFEST_ROOT: &str = "manifests/guided";
pub const GUIDED_MANIFEST_SUFFIX: &str = ".guided.json";

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ProtocolClass {
    Query,
    JsonTarget,
    RestXml,
    RestJson,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GuidedManifest {
    pub schema_version: String,
    pub service: String,
    pub protocol: ProtocolClass,
    pub flows: Vec<GuidedFlow>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GuidedFlow {
    pub id: String,
    pub level: String,
    pub steps: Vec<GuidedStep>,
    #[serde(default)]
    pub cleanup: Vec<GuidedStep>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GuidedStep {
    pub id: String,
    pub title: String,
    pub operation: NormalizedOperation,
    #[serde(default)]
    pub assertions: Vec<FlowAssertion>,
    #[serde(default)]
    pub captures: Vec<CaptureBinding>,
    pub error_guidance: Option<String>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct NormalizedOperation {
    pub method: String,
    pub path: String,
    #[serde(default)]
    pub headers: HashMap<String, String>,
    #[serde(default)]
    pub query: HashMap<String, String>,
    pub body: Option<String>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct FlowAssertion {
    pub kind: String,
    pub target: String,
    pub expected: String,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CaptureBinding {
    pub name: String,
    pub source: String,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ValidationIssue {
    pub path: String,
    pub message: String,
}

#[derive(Debug, Error)]
pub enum ManifestError {
    #[error("schema document is invalid JSON: {0}")]
    InvalidSchemaDocument(serde_json::Error),
    #[error("manifest is invalid JSON: {0}")]
    InvalidManifestJson(serde_json::Error),
    #[error("manifest structural validation failed")]
    StructuralValidation { issues: Vec<ValidationIssue> },
    #[error("manifest semantic lint failed")]
    SemanticValidation { issues: Vec<ValidationIssue> },
    #[error(
        "unsupported manifest schema version '{manifest_version}' for runtime '{runtime_version}'"
    )]
    IncompatibleSchemaVersion {
        manifest_version: String,
        runtime_version: String,
    },
}

/// Load the embedded JSON schema for guided flow manifests.
///
/// On success returns the parsed `serde_json::Value`. On failure returns
/// `ManifestError::InvalidSchemaDocument` when the embedded schema fails to parse.
///
/// # Examples
///
/// ```
/// let schema = load_schema_document().expect("embedded schema should parse");
/// // schema is a serde_json::Value; common fields like `$id` should be present
/// assert!(schema.get("$id").is_some());
/// ```
pub fn load_schema_document() -> Result<Value, ManifestError> {
    serde_json::from_str(include_str!(
        "../schemas/studio-guided-flow.manifest.v1.schema.json"
    ))
    .map_err(ManifestError::InvalidSchemaDocument)
}

/// Parses a JSON manifest string, validates its structure and semantics, and returns the parsed manifest.
///
/// On success returns the deserialized `GuidedManifest`. On failure returns a `ManifestError` describing
/// parse errors, structural validation problems, semantic lint issues, or schema version incompatibility.
///
/// # Examples
///
/// ```
/// let json = r#"
/// {
///   "schemaVersion": "1.2",
///   "service": "example",
///   "protocol": "rest-json",
///   "flows": [
///     {
///       "id": "f1",
///       "level": "L2",
///       "steps": [
///         {
///           "id": "s1",
///           "title": "step 1",
///           "operation": { "method": "GET", "path": "/ping" }
///         }
///       ]
///     }
///   ]
/// }
/// "#;
///
/// let manifest = parse_and_validate_manifest(json).expect("valid manifest");
/// assert_eq!(manifest.service, "example");
/// ```
pub fn parse_and_validate_manifest(source: &str) -> Result<GuidedManifest, ManifestError> {
    let value: Value = serde_json::from_str(source).map_err(ManifestError::InvalidManifestJson)?;

    let structural_issues = validate_manifest_structure(&value);
    if !structural_issues.is_empty() {
        return Err(ManifestError::StructuralValidation {
            issues: structural_issues,
        });
    }

    let manifest: GuidedManifest =
        serde_json::from_value(value).map_err(ManifestError::InvalidManifestJson)?;
    check_schema_compatibility(&manifest.schema_version, SUPPORTED_SCHEMA_VERSION)?;

    let semantic_issues = lint_manifest_semantics(&manifest);
    if !semantic_issues.is_empty() {
        return Err(ManifestError::SemanticValidation {
            issues: semantic_issues,
        });
    }

    Ok(manifest)
}

/// Validates that a manifest schema version is compatible with the runtime schema version.
///
/// Compatibility requires the major versions to match and the manifest minor version to be
/// less than or equal to the runtime minor version. If either version string cannot be parsed
/// or the compatibility rules are not satisfied, returns `ManifestError::IncompatibleSchemaVersion`.
///
/// # Examples
///
/// ```
/// // Compatible: same major, manifest minor <= runtime minor
/// assert!(crate::guided_manifest::check_schema_compatibility("1.2", "1.3").is_ok());
///
/// // Incompatible: different major
/// assert!(crate::guided_manifest::check_schema_compatibility("1.2", "2.0").is_err());
/// ```
pub fn check_schema_compatibility(
    manifest_version: &str,
    runtime_version: &str,
) -> Result<(), ManifestError> {
    let Some((manifest_major, manifest_minor)) = parse_version(manifest_version) else {
        return Err(ManifestError::IncompatibleSchemaVersion {
            manifest_version: manifest_version.to_string(),
            runtime_version: runtime_version.to_string(),
        });
    };
    let Some((runtime_major, runtime_minor)) = parse_version(runtime_version) else {
        return Err(ManifestError::IncompatibleSchemaVersion {
            manifest_version: manifest_version.to_string(),
            runtime_version: runtime_version.to_string(),
        });
    };

    if manifest_major != runtime_major || manifest_minor > runtime_minor {
        return Err(ManifestError::IncompatibleSchemaVersion {
            manifest_version: manifest_version.to_string(),
            runtime_version: runtime_version.to_string(),
        });
    }

    Ok(())
}

/// Constructs the expected filesystem path for a service's guided manifest.
///
/// The returned path is built from the module's `GUIDED_MANIFEST_ROOT` and
/// `GUIDED_MANIFEST_SUFFIX` constants with the given service name inserted.
///
/// # Examples
///
/// ```
/// let path = expected_manifest_path("payments");
/// assert_eq!(path, "manifests/guided/payments.guided.json");
/// ```
pub fn expected_manifest_path(service: &str) -> String {
    format!("{GUIDED_MANIFEST_ROOT}/{service}{GUIDED_MANIFEST_SUFFIX}")
}

/// Ensures there is exactly one guided manifest per supported service and that all required manifests exist.
///
/// Validates the provided manifest file paths:
/// - Records an issue for any path that does not follow the required manifests/guided/<service>.guided.json pattern.
/// - Records an issue for duplicate manifests for the same service.
/// - Records an issue for any supported service that has no corresponding manifest path.
///
/// # Parameters
///
/// - `services`: List of supported service names that must each have a manifest.
/// - `manifest_paths`: List of manifest file paths to validate.
///
/// # Returns
///
/// `Ok(())` if every supported service has exactly one valid manifest path; `Err(ManifestError::SemanticValidation)` containing `ValidationIssue` entries otherwise.
///
/// # Examples
///
/// ```
/// let services = vec!["s3".to_string(), "ec2".to_string()];
/// let manifests = vec![
///     "manifests/guided/s3.guided.json".to_string(),
///     "manifests/guided/ec2.guided.json".to_string(),
/// ];
/// assert!(enforce_one_manifest_per_service(&services, &manifests).is_ok());
/// ```
pub fn enforce_one_manifest_per_service(
    services: &[String],
    manifest_paths: &[String],
) -> Result<(), ManifestError> {
    let mut issues = Vec::new();
    let mut seen = HashSet::new();

    for path in manifest_paths {
        let Some(name) = service_name_from_path(path) else {
            issues.push(ValidationIssue {
                path: path.clone(),
                message: format!(
                    "manifest path must follow '{GUIDED_MANIFEST_ROOT}/<service>{GUIDED_MANIFEST_SUFFIX}'"
                ),
            });
            continue;
        };

        if !seen.insert(name.clone()) {
            issues.push(ValidationIssue {
                path: path.clone(),
                message: format!("duplicate manifest for service '{name}'"),
            });
        }
    }

    for service in services {
        if !seen.contains(service) {
            issues.push(ValidationIssue {
                path: expected_manifest_path(service),
                message: "missing manifest for supported service".to_string(),
            });
        }
    }

    if issues.is_empty() {
        Ok(())
    } else {
        Err(ManifestError::SemanticValidation { issues })
    }
}

/// Validates the structural shape of a manifest JSON value.
///
/// Performs schema-like checks on the JSON structure (presence and types of required
/// top-level keys, flows, steps, and operation objects) and returns a collection of
/// actionable validation issues describing any structural problems found.
///
/// # Returns
///
/// A `Vec<ValidationIssue>` containing one entry per structural problem; the vector is
/// empty when the manifest structure satisfies all checked requirements.
///
/// # Examples
///
/// ```
/// use serde_json::json;
/// use crate::guided_manifest::{validate_manifest_structure, ValidationIssue};
///
/// // Missing `flows` produces a structural issue
/// let v = json!({
///     "schemaVersion": "1.2",
///     "service": "example",
///     "protocol": "rest-json"
/// });
///
/// let issues = validate_manifest_structure(&v);
/// assert!(!issues.is_empty());
/// assert_eq!(issues[0].path, "$.flows");
/// ```
fn validate_manifest_structure(value: &Value) -> Vec<ValidationIssue> {
    let mut issues = Vec::new();

    let Some(obj) = value.as_object() else {
        issues.push(ValidationIssue {
            path: "$".to_string(),
            message: "manifest root must be an object".to_string(),
        });
        return issues;
    };

    require_string(obj, "schemaVersion", "$", &mut issues);
    require_string(obj, "service", "$", &mut issues);
    require_string(obj, "protocol", "$", &mut issues);

    match obj.get("flows") {
        Some(flows) if flows.is_array() => {
            if let Some(items) = flows.as_array() {
                if items.is_empty() {
                    issues.push(ValidationIssue {
                        path: "$.flows".to_string(),
                        message: "must contain at least one flow".to_string(),
                    });
                }

                for (flow_index, flow) in items.iter().enumerate() {
                    let flow_path = format!("$.flows[{flow_index}]");
                    let Some(flow_obj) = flow.as_object() else {
                        issues.push(ValidationIssue {
                            path: flow_path,
                            message: "flow must be an object".to_string(),
                        });
                        continue;
                    };

                    require_string(flow_obj, "id", &flow_path, &mut issues);
                    require_string(flow_obj, "level", &flow_path, &mut issues);

                    match flow_obj.get("steps") {
                        Some(steps) if steps.is_array() => {
                            if let Some(step_items) = steps.as_array() {
                                if step_items.is_empty() {
                                    issues.push(ValidationIssue {
                                        path: format!("{flow_path}.steps"),
                                        message: "must contain at least one step".to_string(),
                                    });
                                }

                                for (step_index, step) in step_items.iter().enumerate() {
                                    let step_path = format!("{flow_path}.steps[{step_index}]");
                                    let Some(step_obj) = step.as_object() else {
                                        issues.push(ValidationIssue {
                                            path: step_path,
                                            message: "step must be an object".to_string(),
                                        });
                                        continue;
                                    };

                                    require_string(step_obj, "id", &step_path, &mut issues);
                                    require_string(step_obj, "title", &step_path, &mut issues);

                                    match step_obj.get("operation") {
                                        Some(op) if op.is_object() => {
                                            if let Some(op_obj) = op.as_object() {
                                                let op_path = format!("{step_path}.operation");
                                                require_string(
                                                    op_obj,
                                                    "method",
                                                    &op_path,
                                                    &mut issues,
                                                );
                                                require_string(
                                                    op_obj,
                                                    "path",
                                                    &op_path,
                                                    &mut issues,
                                                );
                                            }
                                        }
                                        Some(_) => issues.push(ValidationIssue {
                                            path: format!("{step_path}.operation"),
                                            message: "must be an object".to_string(),
                                        }),
                                        None => issues.push(ValidationIssue {
                                            path: format!("{step_path}.operation"),
                                            message: "is required".to_string(),
                                        }),
                                    }
                                }
                            }
                        }
                        Some(_) => issues.push(ValidationIssue {
                            path: format!("{flow_path}.steps"),
                            message: "must be an array".to_string(),
                        }),
                        None => issues.push(ValidationIssue {
                            path: format!("{flow_path}.steps"),
                            message: "is required".to_string(),
                        }),
                    }
                }
            }
        }
        Some(_) => issues.push(ValidationIssue {
            path: "$.flows".to_string(),
            message: "must be an array".to_string(),
        }),
        None => issues.push(ValidationIssue {
            path: "$.flows".to_string(),
            message: "is required".to_string(),
        }),
    }

    issues
}

/// Performs semantic validation of a guided manifest and collects any issues found.
///
/// The function checks each flow and its steps for semantic rules such as:
/// - Each flow must contain at least one assertion across its steps.
/// - Flows with level `l1` must include cleanup steps.
/// - Interpolations in operation path, headers, query parameters, and body are validated.
///
/// # Returns
///
/// A `Vec<ValidationIssue>` containing one entry per semantic problem detected. If no issues
/// are found the returned vector is empty.
///
/// # Examples
///
/// ```
/// use std::collections::HashMap;
///
/// let manifest = GuidedManifest {
///     schema_version: "1.2".to_string(),
///     service: "svc".to_string(),
///     protocol: ProtocolClass::RestJson,
///     flows: vec![
///         GuidedFlow {
///             id: "f1".to_string(),
///             level: "L2".to_string(),
///             steps: vec![
///                 GuidedStep {
///                     id: "s1".to_string(),
///                     title: "step".to_string(),
///                     operation: NormalizedOperation {
///                         method: "GET".to_string(),
///                         path: "/".to_string(),
///                         headers: HashMap::new(),
///                         query: HashMap::new(),
///                         body: None,
///                     },
///                     assertions: vec![FlowAssertion { kind: "status".to_string(), target: "200".to_string(), expected: "200".to_string() }],
///                     captures: vec![],
///                     error_guidance: None,
///                 }
///             ],
///             cleanup: vec![],
///         }
///     ],
/// };
///
/// let issues = lint_manifest_semantics(&manifest);
/// assert!(issues.is_empty());
/// ```
fn lint_manifest_semantics(manifest: &GuidedManifest) -> Vec<ValidationIssue> {
    let mut issues = Vec::new();

    for (flow_index, flow) in manifest.flows.iter().enumerate() {
        let flow_path = format!("$.flows[{flow_index}]");
        let is_l1 = flow.level.eq_ignore_ascii_case("l1");

        let assertion_count: usize = flow.steps.iter().map(|s| s.assertions.len()).sum();
        if assertion_count == 0 {
            issues.push(ValidationIssue {
                path: flow_path.clone(),
                message: "flow must define at least one assertion".to_string(),
            });
        }

        if is_l1 && flow.cleanup.is_empty() {
            issues.push(ValidationIssue {
                path: format!("{flow_path}.cleanup"),
                message: "L1 flow must define cleanup steps".to_string(),
            });
        }

        for (step_index, step) in flow.steps.iter().enumerate() {
            let step_path = format!("{flow_path}.steps[{step_index}]");
            check_interpolation_value(
                &step.operation.path,
                &format!("{step_path}.operation.path"),
                &mut issues,
            );

            for (name, value) in &step.operation.headers {
                check_interpolation_value(
                    value,
                    &format!("{step_path}.operation.headers.{name}"),
                    &mut issues,
                );
            }

            for (name, value) in &step.operation.query {
                check_interpolation_value(
                    value,
                    &format!("{step_path}.operation.query.{name}"),
                    &mut issues,
                );
            }

            if let Some(body) = &step.operation.body {
                check_interpolation_value(
                    body,
                    &format!("{step_path}.operation.body"),
                    &mut issues,
                );
            }
        }
    }

    issues
}

/// Scans a string for `{{ ... }}` interpolation tokens, validates each expression,
/// and appends any discovered validation issues to `issues`.
///
/// The `path` is recorded on each produced `ValidationIssue` so callers can
/// associate the problem with the originating manifest location. This function
/// records an issue for an unclosed token (`"unclosed interpolation expression"`)
/// and for any expression that `validate_expression` deems invalid.
///
/// # Examples
///
/// ```
/// let mut issues = Vec::new();
/// check_interpolation_value("prefix {{ missing_end", "flows[0].steps[0].operation.path", &mut issues);
/// assert_eq!(issues.len(), 1);
/// assert_eq!(issues[0].message, "unclosed interpolation expression");
/// assert_eq!(issues[0].path, "flows[0].steps[0].operation.path");
/// ```
fn check_interpolation_value(value: &str, path: &str, issues: &mut Vec<ValidationIssue>) {
    let mut cursor = 0usize;
    while let Some(start) = value[cursor..].find("{{") {
        let absolute_start = cursor + start;
        let token_start = absolute_start + 2;
        let Some(end_rel) = value[token_start..].find("}}") else {
            issues.push(ValidationIssue {
                path: path.to_string(),
                message: "unclosed interpolation expression".to_string(),
            });
            return;
        };

        let token_end = token_start + end_rel;
        let expr = value[token_start..token_end].trim();
        if let Err(message) = validate_expression(expr) {
            issues.push(ValidationIssue {
                path: path.to_string(),
                message,
            });
        }

        cursor = token_end + 2;
    }
}

/// Validates a single interpolation expression for allowed sources and characters.
///
/// The function accepts built-in calls `rand8()` and `timestamp()`, or dotted identifier
/// paths that start with `inputs.`, `context.`, or `captures.` where every segment is a valid
/// identifier segment (letters or underscore to start, then ASCII alphanumeric, underscore, or hyphen).
/// It rejects empty expressions, expressions containing unsafe characters (`;`, `` ` ``, or `${`),
/// and any other unsupported sources.
///
/// # Returns
///
/// `Ok(())` if the expression is valid, `Err(String)` with a diagnostic message otherwise.
///
/// # Examples
///
/// ```
/// assert!(validate_expression("rand8()").is_ok());
/// assert!(validate_expression("timestamp()").is_ok());
/// assert!(validate_expression("inputs.user.id").is_ok());
/// assert!(validate_expression("captures.session-1.token").is_ok());
///
/// assert!(validate_expression("").is_err());
/// assert!(validate_expression("rm -rf /;").is_err());
/// assert!(validate_expression("unknown.source").is_err());
/// ```
fn validate_expression(expr: &str) -> Result<(), String> {
    if expr.is_empty() {
        return Err("empty interpolation expression".to_string());
    }

    if expr.contains(';') || expr.contains('`') || expr.contains("${") {
        return Err(format!("unsafe expression '{expr}'"));
    }

    if expr == "rand8()" || expr == "timestamp()" {
        return Ok(());
    }

    if expr.starts_with("inputs.") || expr.starts_with("context.") || expr.starts_with("captures.")
    {
        if expr.split('.').all(is_valid_identifier_segment) {
            return Ok(());
        }
        return Err(format!("invalid identifier path in expression '{expr}'"));
    }

    Err(format!(
        "unsupported expression source '{expr}', expected inputs/context/captures/built-ins"
    ))
}

/// Determines whether a string is a valid identifier segment.
///
/// A valid segment is non-empty, begins with an ASCII letter (A–Z, a–z) or underscore (`_`),
/// and the remaining characters (if any) are ASCII alphanumeric, underscore (`_`), or hyphen (`-`).
///
/// # Returns
///
/// `true` if the segment meets the rules above, `false` otherwise.
///
/// # Examples
///
/// ```
/// assert!(is_valid_identifier_segment("abc"));
/// assert!(is_valid_identifier_segment("_foo-1"));
/// assert!(!is_valid_identifier_segment("1abc"));
/// assert!(!is_valid_identifier_segment(""));
/// ```
fn is_valid_identifier_segment(segment: &str) -> bool {
    if segment.is_empty() {
        return false;
    }

    let mut chars = segment.chars();
    let Some(first) = chars.next() else {
        return false;
    };
    if !first.is_ascii_alphabetic() && first != '_' {
        return false;
    }

    chars.all(|ch| ch.is_ascii_alphanumeric() || ch == '_' || ch == '-')
}

/// Parses a version string and extracts the major and minor numeric components.
///
/// The input should be in the form "MAJOR.MINOR" where both MAJOR and MINOR are unsigned integers.
/// If the string does not contain two dot-separated numeric parts, `None` is returned.
///
/// # Returns
///
/// `Some((major, minor))` when both components parse as `u64`, `None` otherwise.
///
/// # Examples
///
/// ```
/// assert_eq!(parse_version("1.2"), Some((1u64, 2u64)));
/// assert_eq!(parse_version("10.0"), Some((10, 0)));
/// assert_eq!(parse_version("1"), None);         // missing minor
/// assert_eq!(parse_version("a.b"), None);       // non-numeric
/// assert_eq!(parse_version("1.2.3"), Some((1, 2))); // extra parts are ignored
/// ```
fn parse_version(value: &str) -> Option<(u64, u64)> {
    let mut parts = value.split('.');
    let major = parts.next()?.parse::<u64>().ok()?;
    let minor = parts.next()?.parse::<u64>().ok()?;
    Some((major, minor))
}

/// Ensures a named key exists in a JSON object and that its value is a string, recording validation issues when it is missing or the wrong type.
///
/// If the key is missing, appends a `ValidationIssue` with message `"is required"` and path `"{path}.{key}"`.
/// If the key exists but is not a string, appends a `ValidationIssue` with message `"must be a string"` and path `"{path}.{key}"`.
///
/// # Parameters
///
/// - `object`: JSON object to check.
/// - `key`: Key name to require in the object.
/// - `path`: Base JSON path used when constructing the `ValidationIssue` path.
/// - `issues`: Mutable vector to which any discovered `ValidationIssue`s are appended.
///
/// # Examples
///
/// ```
/// use serde_json::{Map, Value};
/// let mut map = Map::new();
/// map.insert("name".to_string(), Value::String("svc".to_string()));
/// let mut issues = Vec::new();
/// require_string(&map, "name", "root", &mut issues);
/// assert!(issues.is_empty());
///
/// let mut map2 = Map::new();
/// map2.insert("id".to_string(), Value::Number(1.into()));
/// let mut issues2 = Vec::new();
/// require_string(&map2, "id", "flow", &mut issues2);
/// assert_eq!(issues2.len(), 1);
/// assert_eq!(issues2[0].path, "flow.id");
/// assert_eq!(issues2[0].message, "must be a string");
/// ```
fn require_string(
    object: &serde_json::Map<String, Value>,
    key: &str,
    path: &str,
    issues: &mut Vec<ValidationIssue>,
) {
    match object.get(key) {
        Some(v) if v.is_string() => {}
        Some(_) => issues.push(ValidationIssue {
            path: format!("{path}.{key}"),
            message: "must be a string".to_string(),
        }),
        None => issues.push(ValidationIssue {
            path: format!("{path}.{key}"),
            message: "is required".to_string(),
        }),
    }
}

/// Extracts the service name from a guided manifest file path when the path follows the expected layout.
///
/// The function returns the file-stem (the filename with `GUIDED_MANIFEST_SUFFIX` removed) if:
/// - the filename ends with `GUIDED_MANIFEST_SUFFIX`,
/// - the resulting service name is non-empty,
/// - and the path contains `GUIDED_MANIFEST_ROOT`.
/// Otherwise it returns `None`.
///
/// # Examples
///
/// ```
/// // Unix-style path
/// let p = "manifests/guided/my-service.guided.json";
/// assert_eq!(service_name_from_path(p), Some("my-service".to_string()));
///
/// // Windows-style separators are normalized
/// let p2 = r"manifests\\guided\\other-service.guided.json";
/// assert_eq!(service_name_from_path(p2), Some("other-service".to_string()));
///
/// // Missing root or wrong suffix -> None
/// assert_eq!(service_name_from_path("other/dir/service.json"), None);
/// ```
fn service_name_from_path(path: &str) -> Option<String> {
    let normalized = path.replace('\\', "/");
    let file_name = Path::new(&normalized).file_name()?.to_str()?;
    if !file_name.ends_with(GUIDED_MANIFEST_SUFFIX) {
        return None;
    }

    let service = file_name.strip_suffix(GUIDED_MANIFEST_SUFFIX)?;
    if service.is_empty() {
        return None;
    }

    if !normalized.contains(GUIDED_MANIFEST_ROOT) {
        return None;
    }

    Some(service.to_string())
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn schema_document_loads() {
        let schema = load_schema_document().expect("schema should parse");
        assert_eq!(schema["$id"], "studio-guided-flow.manifest.v1");
    }

    #[test]
    fn parser_rejects_missing_required_fields_with_actionable_path() {
        let input = r#"{"schemaVersion":"1.0","service":"s3"}"#;
        let err = parse_and_validate_manifest(input).expect_err("missing fields must fail");

        match err {
            ManifestError::StructuralValidation { issues } => {
                assert!(issues.iter().any(|issue| issue.path == "$.protocol"));
                assert!(issues.iter().any(|issue| issue.path == "$.flows"));
            }
            other => panic!("unexpected error: {other:?}"),
        }
    }

    #[test]
    fn semantic_lint_rejects_unsafe_expression_and_missing_l1_cleanup() {
        let input = r#"
        {
          "schemaVersion":"1.0",
          "service":"s3",
          "protocol":"rest_xml",
          "flows":[
            {
              "id":"basic",
              "level":"L1",
              "steps":[
                {
                  "id":"create",
                  "title":"Create",
                  "operation":{
                    "method":"PUT",
                    "path":"/{{evil()}}",
                    "headers":{},
                    "query":{}
                  },
                  "assertions":[]
                }
              ]
            }
          ]
        }
        "#;

        let err = parse_and_validate_manifest(input).expect_err("semantic lint must fail");
        match err {
            ManifestError::SemanticValidation { issues } => {
                assert!(
                    issues
                        .iter()
                        .any(|issue| issue.message.contains("unsupported expression source"))
                );
                assert!(
                    issues
                        .iter()
                        .any(|issue| issue.message.contains("L1 flow must define cleanup steps"))
                );
                assert!(
                    issues
                        .iter()
                        .any(|issue| issue.message.contains("at least one assertion"))
                );
            }
            other => panic!("unexpected error: {other:?}"),
        }
    }

    #[test]
    fn schema_compatibility_rules_follow_major_minor_contract() {
        assert!(check_schema_compatibility("1.0", "1.2").is_ok());
        assert!(check_schema_compatibility("1.2", "1.2").is_ok());
        assert!(check_schema_compatibility("1.3", "1.2").is_err());
        assert!(check_schema_compatibility("2.0", "1.2").is_err());
    }

    #[test]
    fn one_manifest_per_service_enforced_by_layout() {
        let services = vec!["s3".to_string(), "sqs".to_string()];
        let manifests = vec![
            "manifests/guided/s3.guided.json".to_string(),
            "manifests/guided/s3.guided.json".to_string(),
        ];

        let err = enforce_one_manifest_per_service(&services, &manifests).expect_err("must fail");
        match err {
            ManifestError::SemanticValidation { issues } => {
                assert!(
                    issues
                        .iter()
                        .any(|issue| issue.message.contains("duplicate manifest"))
                );
                assert!(
                    issues
                        .iter()
                        .any(|issue| issue.message.contains("missing manifest"))
                );
            }
            other => panic!("unexpected error: {other:?}"),
        }
    }
}
