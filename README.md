...
- Branch policy: open PRs into `develop`; do **not** open `develop` -> `main` PRs. Promote `develop` to `main` via **fast-forward only** using the dedicated workflow/CLI path (no merge/squash/rebase into `main`).
